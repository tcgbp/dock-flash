...
**Floating windows all share one z-index.** Every `.dsh-wb-floating` gets `z-index: 70` from a single CSS rule; dock-base's inline style carries only `left/top/width/height`, and there is **no focus-based promotion**. dock-base renders them as `Object.values(layout.floatingWindows).map(...)`, and for equal z-index the **last one in that object paints on top**. So another plugin's floating window (reported with dock-git's graph) permanently covers ours, and clicking ours never raises it.

There is no z-index lever here — the whole window is dock-base's element, not ours. The only lever from inside the panel is **reordering that object**, which `promoteFloatingWindow` in `QuickControlPanel` does: it moves our own entry to the end and calls `wb.updateLayout({ floatingWindows })`. That works because `store.update()` is a shallow merge of arbitrary keys that re-renders and persists.

It bails out when our entry is already last, which is what stops it from looping — `updateLayout()` notifies the layout listeners, and this panel subscribes via `onDidChangeLayout`, so an unguarded call would re-render forever. It is invoked on becoming `active` and on `onMouseEnter` of the panel root, and is a no-op when our view is docked or closed.

Note this does **not** help the docked case: a floating window legitimately sits above `.dsh-wb-sidebar` (70 > 59 in dock mode), and that precedence is dock-base's design. Do not try to "fix" it by pushing our docked panel above 70.
...
| 1.0.7 | The real occluder turned out to be `.dsh-wb-floating` — dock-base's floating window, i.e. another plugin's window (dock-git's graph) floating above ours, which the 1.0.6 z-index change could not touch because that element is dock-base's, not ours, and every `.dsh-wb-floating` shares one CSS `z-index: 70` with no focus-based promotion. Added `promoteFloatingWindow` to `QuickControlPanel`: it moves our own entry to the end of `layout.floatingWindows` and calls `wb.updateLayout({ floatingWindows })`, since dock-base renders those windows as `Object.values(...)` and the last one wins at equal z-index. It bails out when we are already last (otherwise `onDidChangeLayout` would re-render in a loop) and is a no-op when our view is docked or closed. Invoked on becoming `active` and on `onMouseEnter` of the panel root. |
