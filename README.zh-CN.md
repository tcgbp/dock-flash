...
| **独立模式** | 未安装 dock-base | ⚡ 徽标注入侧边栏底部 → 浮动面板 | 外观 + 布局（触发位置）+ 系统 |
...
> **失焦关闭**不在此列表中。它是面板标题栏上的一个切换按钮，位于关闭（×）按钮左侧，两种模式下都有。

> **停靠布局不在这里配置。** 停靠边、自动隐藏、保留空间、图标缩放都是 dock-base 自己的设置项 —— dock-flash 有意不重复提供。dock-flash 唯一拥有的布局开关是 `trigger-position`，且仅独立模式存在。
...
**无 dock-base**：dock-flash 以独立模式运行 — ⚡ 徽标注入侧边栏底部，点击展开快捷控制浮动面板（外观、布局（触发位置）、系统开关）。
