...
| 模式 | 条件 | 界面 | 可用开关 |
| --- | --- | --- | --- |
| **工作台模式** | 已安装 dock-base | ⚡ 图标在活动栏 → 侧边栏/浮窗面板 | 全部开关（外观 + 布局 + 系统） |
| **独立模式** | 未安装 dock-base | ⚡ 悬浮按钮（右下角）→ 弹出面板 | 外观 + 系统开关（无布局开关） |
...
| 分组 | 开关 | 类型 | 说明 | 独立模式 |
| --- | --- | --- | --- | --- |
| 🎨 外观 | 页面缩放 | slider | 50%–200%，通过 CSS `zoom` 控制全局缩放 | ✅ |
...
| 📐 布局 | 停靠位置 | buttongroup | 左 / 右 / 上 / 下，切换工作台停靠边 | ❌ |
| 📐 布局 | 自动隐藏 | toggle | 鼠标离开时自动隐藏 / 关闭 | ❌ |
| 📐 布局 | 失焦关闭 | buttongroup | 关闭 / 开启，点击面板外部时自动关闭浮窗 | ❌ |
...
- **⚡ 工作台** — 内置开关（id 以 `dock-flash:` 开头），按 `group` 字段分为外观 / 布局 / 系统三个子组
...
**无 dock-base**：dock-flash 以独立模式运行 — 右下角出现⚡悬浮按钮，点击展开快捷控制弹出面板（仅外观 + 系统开关，无布局控制）。

**有 dock-base**：dock-flash 集成到工作台 — ⚡图标出现在活动栏，面板可作为侧边栏或浮动窗口打开，包含全部开关（含布局控制）。
