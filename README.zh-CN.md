...
| ⚙️ 系统 | 测试 URL | select | Google 204 / GitHub / DeepSeek API / 自定义 —— 测试连接所探测的地址。仅一行「标签 + 下拉框」，未配置代理时与「测试连接」一并隐藏 | ✅ |
...
- 🧮 **面板排序** — 工作台与扩展页签头部的 ⇅ 图标进入排序模式：用 ▲▼ 调整分组与开关的先后，结果按浏览器持久化。声明了 `cluster` 的开关整簇作为一个单位移动、簇内顺序固定 —— 系统代理那组就是动因：它四个开关里有三个要配置了代理才出现
...
interface QuickSwitchDefinition {
  ...
  group?: string
  /**
   * 可选的簇标签。共享同一标签的开关在排序时视为一个单位（共用一组 ▲▼），
   * 簇内顺序固定为各自的 `order` 值。适用于"必须待在一起才有意义"的控件，
   * 尤其是其中部分会被 `visible` 条件隐藏的情况。
   */
  cluster?: string
