...
| ⚙️ 系统 | 使用 Windows 系统代理 | toggle | **仅当**未设置任何代理环境变量、且 Windows 配了手动代理（设置 → 网络和 Internet → 代理）时出现。Node 并不读取该设置，所以只读取不够 —— 打开此开关才会真正应用它 | ✅ |
| ⚙️ 系统 | 系统代理 | select | 全部代理 / 仅 API 绕过 / 全部绕过 / 自定义 — 细粒度 NO_PROXY 控制。仅在代理生效时显示 | ✅ |
