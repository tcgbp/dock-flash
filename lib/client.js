...
//  #region AlertDisplayModes   — Toast, Dropdown, Hover, Banner display modes
...
//#region i18n ─────────────────────────────────────────────────────────────────
    ...
    const en = {
      ...
      alertHover: 'Hover Preview',
      alertBanner: 'Scrolling Banner',
...
//#region Styles ───────────────────────────────────────────────────────────────
    ...
    const S = {
      ...
      toastCardCritical: { borderLeftColor: '#e5534b' },
      toastCardError: { borderLeftColor: '#d4764e' },
      toastCardWarning: { borderLeftColor: '#c69026' },
      toastCardInfo: { borderLeftColor: '#3884ff' },
      toastIcon: {
        fontSize: '14px',
        flexShrink: 0,
        lineHeight: '1.4',
...
//#endregion ───────────────────────────────────────────────────────────────────
    // ═══════════════════════════════════════════════════════════════════════
//#region AlertDisplayModes ────────────────────────────────────────────────────
    // Four independent alert display modes: Toast, Dropdown, Hover, Banner.
    ...
    var ALERT_DISPLAY_KEYS = {
      ...
      hover: 'dock-flash:alert-hover',
      banner: 'dock-flash:alert-banner',
    }
    var ALERT_DISPLAY_DEFAULTS = { toast: true, dropdown: false, hover: false, banner: false }
    ...
    // ── Severity helpers ───────────────────────────────────────────────────
    var SEV_COLORS = { critical: '#e5534b', error: '#d4764e', warning: '#c69026', info: '#3884ff' }
    ...
    function _showToast(alert) {
      ...
      var entry = { id: alert.id, el: card, timer: null }
      ...
      if (!alerts || alerts.length === 0) {
        ...
        for (var i = 0; i < alerts.length; i++) {
          (function (alert) {
            ...
            item.addEventListener('mouseenter', function () { item.style.background = 'rgba(127,127,127,0.08)' })
    ...
    // ════════════════════════════════════════════════════════════════════════
    // 3. HOVER PREVIEW (tooltip list on mouse hover over trigger)
    // ════════════════════════════════════════════════════════════════════════
    var _hoverEl = null
    var _hoverVisible = false
    var _hoverShowTimer = null
    var _hoverHideTimer = null
    var _hoverListenersAttached = false
    var _hoverEnterHandler = null
    var _hoverLeaveHandler = null

    function _createHoverTip(alerts) {
      if (_hoverEl && _hoverEl.parentNode) {
        _hoverEl.parentNode.removeChild(_hoverEl)
      }
      _hoverEl = document.createElement('div')
      _hoverEl.setAttribute('data-dock-flash-alert-hover', '')
      Object.keys(S.alertHoverTip).forEach(function (k) {
        _hoverEl.style[k] = S.alertHoverTip[k]
      })

      // Show top 5 alerts by severity
      var topAlerts = (alerts || []).slice(0, 5)
      for (var i = 0; i < topAlerts.length; i++) {
        var alert = topAlerts[i]
        var sev = alert.severity || 'info'
        var item = document.createElement('div')
        Object.keys(S.alertHoverItem).forEach(function (k) { item.style[k] = S.alertHoverItem[k] })

        var dot = document.createElement('span')
        Object.keys(S.alertHoverSevDot).forEach(function (k) { dot.style[k] = S.alertHoverSevDot[k] })
        dot.style.background = SEV_COLORS[sev] || SEV_COLORS.info
        item.appendChild(dot)

        var iconSpan = document.createElement('span')
        Object.keys(S.alertHoverItemIcon).forEach(function (k) { iconSpan.style[k] = S.alertHoverItemIcon[k] })
        iconSpan.textContent = alert.icon || SEV_ICONS[sev] || '⚡'
        item.appendChild(iconSpan)

        var titleSpan = document.createElement('span')
        Object.keys(S.alertHoverItemTitle).forEach(function (k) { titleSpan.style[k] = S.alertHoverItemTitle[k] })
        titleSpan.textContent = _resolveAlertText(alert.title)
        item.appendChild(titleSpan)

        _hoverEl.appendChild(item)
      }

      return _hoverEl
    }

    function _positionHoverTip() {
      if (!_hoverEl) return
      var rect = _getTriggerRect()
      if (!rect) {
        _hoverEl.style.top = '60px'
        _hoverEl.style.right = '12px'
      } else {
        _hoverEl.style.top = (rect.bottom + 4) + 'px'
        _hoverEl.style.left = Math.max(4, rect.left) + 'px'
        var hw = 220
        if (rect.left + hw > window.innerWidth - 8) {
          _hoverEl.style.left = Math.max(4, window.innerWidth - hw - 8) + 'px'
        }
      }
    }

    function _showHoverTip(alertRegistry) {
      if (_hoverVisible) return
      var alerts = alertRegistry.getAlerts()
      if (!alerts || alerts.length === 0) return
      _createHoverTip(alerts)
      _positionHoverTip()
      document.body.appendChild(_hoverEl)
      // Trigger opacity transition
      requestAnimationFrame(function () {
        if (_hoverEl) {
          Object.keys(S.alertHoverTipVisible).forEach(function (k) {
            _hoverEl.style[k] = S.alertHoverTipVisible[k]
          })
        }
      })
      _hoverVisible = true
    ...
    function _startHoverMode(alertRegistry) {
      var triggerEl = document.querySelector('[data-dock-flash-trigger]')
      if (!triggerEl) {
        // Retry once after a delay
        setTimeout(function () {
          if (!_alertDisplayActive.hover) return  // mode may have been stopped
          triggerEl = document.querySelector('[data-dock-flash-trigger]')
          if (triggerEl) _attachHoverListeners(triggerEl, alertRegistry)
        }, 2000)
        return
      }
      _attachHoverListeners(triggerEl, alertRegistry)
    }

    function _attachHoverListeners(triggerEl, alertRegistry) {
      if (_hoverListenersAttached) return  // prevent duplicate listeners
      _hoverListenersAttached = true
      _hoverEnterHandler = function () {
        if (!_getAlertDisplayMode('hover')) return  // mode may have been turned off
        if (_hoverHideTimer) { clearTimeout(_hoverHideTimer); _hoverHideTimer = null }
        _hoverShowTimer = setTimeout(function () { _showHoverTip(alertRegistry) }, 300)
      }
      _hoverLeaveHandler = function () {
        if (_hoverShowTimer) { clearTimeout(_hoverShowTimer); _hoverShowTimer = null }
        _hoverHideTimer = setTimeout(_hideHoverTip, 200)
      }
      triggerEl.addEventListener('mouseenter', _hoverEnterHandler)
      triggerEl.addEventListener('mouseleave', _hoverLeaveHandler)
    }

    function _stopHoverMode() {
      _alertDisplayActive.hover = false
      _hideHoverTip()
      if (_hoverShowTimer) { clearTimeout(_hoverShowTimer); _hoverShowTimer = null }
      if (_hoverHideTimer) { clearTimeout(_hoverHideTimer); _hoverHideTimer = null }
      // Remove hover listeners from the trigger element
      if (_hoverListenersAttached) {
        var triggerEl = document.querySelector('[data-dock-flash-trigger]')
        if (triggerEl) {
          if (_hoverEnterHandler) triggerEl.removeEventListener('mouseenter', _hoverEnterHandler)
          if (_hoverLeaveHandler) triggerEl.removeEventListener('mouseleave', _hoverLeaveHandler)
        }
        _hoverListenersAttached = false
    ...
    // ════════════════════════════════════════════════════════════════════════
    // 4. SCROLLING BANNER (multiple thin marquee bars below trigger)
    //    One banner per active alert, stacked vertically.
    //    Each banner is colored by its own severity level.
    //    Max 5 banners shown (highest severity first).
    // ════════════════════════════════════════════════════════════════════════
    var _bannerContainer = null    // wrapper div holding all banner rows
    var _bannerRows = []           // { el, textEl, scrollPos }
    var _bannerUnsub = null
    var _bannerAnimFrame = null
    var _bannerPositionTimer = null
    var BANNER_MAX_ROWS = 5

    function _removeAllBanners() {
      console.log('[dock-flash] _removeAllBanners: rows=' + _bannerRows.length + ', container=' + !!_bannerContainer)
      for (var i = 0; i < _bannerRows.length; i++) {
        if (_bannerRows[i].el && _bannerRows[i].el.parentNode) {
          _bannerRows[i].el.parentNode.removeChild(_bannerRows[i].el)
        }
      }
      _bannerRows = []
      if (_bannerContainer && _bannerContainer.parentNode) {
        _bannerContainer.parentNode.removeChild(_bannerContainer)
      }
      _bannerContainer = null
      // Also clean up any orphaned banner elements left in the DOM
      // (defensive: ensures no stale banners survive a missed cleanup)
      var orphans = document.querySelectorAll('[data-dock-flash-alert-banner-container]')
      if (orphans.length > 0) {
        console.warn('[dock-flash] _removeAllBanners: found ' + orphans.length + ' orphaned banner containers')
        for (var j = 0; j < orphans.length; j++) {
          if (orphans[j].parentNode) orphans[j].parentNode.removeChild(orphans[j])
        }
      }
    }

    function _repositionBannerContainer() {
      if (!_bannerContainer || !_alertDisplayActive.banner) return
      var rect = _getTriggerRect()
      if (rect) {
        _bannerContainer.style.top = (rect.bottom + 2) + 'px'
        _bannerContainer.style.left = Math.max(4, rect.left) + 'px'
        _bannerContainer.style.right = ''
      }
      // If no trigger yet, keep the container hidden until one appears
      if (!rect) {
        _bannerContainer.style.display = 'none'
      } else {
        _bannerContainer.style.display = ''
      }
    }

    function _createBanners(alerts) {
      _removeAllBanners()
      if (!alerts || alerts.length === 0) return

      // Container: fixed position, stacked column
      _bannerContainer = document.createElement('div')
      _bannerContainer.setAttribute('data-dock-flash-alert-banner-container', '')
      _bannerContainer.style.cssText = 'position:fixed;z-index:1199;display:flex;flex-direction:column;gap:2px;pointer-events:none;'

      var shown = alerts.slice(0, BANNER_MAX_ROWS)
      for (var i = 0; i < shown.length; i++) {
        var alert = shown[i]
        var sev = alert.severity || 'info'
        var sevColor = SEV_COLORS[sev] || SEV_COLORS.info

        var row = document.createElement('div')
        row.setAttribute('data-dock-flash-alert-banner', '')
        Object.keys(S.alertBanner).forEach(function (k) { row.style[k] = S.alertBanner[k] })
        row.style.background = sevColor
        row.style.pointerEvents = 'auto'  // allow click on individual banners
        var textEl = document.createElement('span')
        textEl.textContent = (alert.icon || SEV_ICONS[sev] || '⚡') + ' ' + _resolveAlertText(alert.title)
        textEl.style.cssText = 'display:inline-block;padding-right:40px;white-space:nowrap;'
        row.appendChild(textEl)

        // Click to open panel
        ;(function (a) {
          row.addEventListener('click', function () { _openPanelToSystem() })
        })(alert)

        _bannerContainer.appendChild(row)
        // Stagger scroll position per row so they don't all move in sync
        _bannerRows.push({ el: row, textEl: textEl, scrollPos: (i + 1) * 40 })
      }

      // Position: try to anchor below the trigger button.
      // If the trigger hasn't mounted yet, hide the container;
      // a periodic reposition will show it once the trigger appears.
      _repositionBannerContainer()
      document.body.appendChild(_bannerContainer)
    }

    function _animateBanners() {
      if (!_alertDisplayActive.banner) return  // mode was stopped
      for (var i = 0; i < _bannerRows.length; i++) {
        var r = _bannerRows[i]
        if (!r.textEl || !r.el) continue
        r.scrollPos -= 0.5
        if (r.textEl.offsetWidth > 0 && r.scrollPos < -r.textEl.offsetWidth) {
          r.scrollPos = r.el.offsetWidth
        }
        r.textEl.style.transform = 'translateX(' + r.scrollPos + 'px)'
      }
      _bannerAnimFrame = requestAnimationFrame(_animateBanners)
    }

    function _startBannerMode(alertRegistry) {
      var alerts = alertRegistry.getAlerts()
      _createBanners(alerts)
      if (_bannerRows.length > 0) {
        _bannerAnimFrame = requestAnimationFrame(_animateBanners)
      }

      // Periodic reposition: keeps banners anchored below the trigger
      // even when it hasn't mounted yet or moves (overlay drag, resize).
      _bannerPositionTimer = setInterval(_repositionBannerContainer, 1000)

      _bannerUnsub = alertRegistry.subscribe(function () {
        if (!_alertDisplayActive.banner) return  // mode may have been stopped
        var alerts = alertRegistry.getAlerts()
        console.log('[dock-flash] banner subscribe: alerts=' + alerts.length)
        if (_bannerAnimFrame) { cancelAnimationFrame(_bannerAnimFrame); _bannerAnimFrame = null }
        _removeAllBanners()
        if (alerts.length === 0) {
          // No active alerts — stop animation, container is already removed
          return
        }
        _createBanners(alerts)
        if (_bannerRows.length > 0) {
    ...
    function _stopBannerMode() {
      // Clear active flag FIRST so any in-flight subscribe callback
      // or timer can bail out early.
      _alertDisplayActive.banner = false
      if (_bannerAnimFrame) { cancelAnimationFrame(_bannerAnimFrame); _bannerAnimFrame = null }
      if (_bannerPositionTimer) { clearInterval(_bannerPositionTimer); _bannerPositionTimer = null }
      if (_bannerUnsub) { try { _bannerUnsub() } catch (_) {}; _bannerUnsub = null }
      _removeAllBanners()
    }
    ...
    var _alertDisplayActive = { toast: false, dropdown: false, hover: false, banner: false }
    ...
    function _startAlertDisplayMode(mode) {
      ...
      switch (mode) {
        ...
        case 'hover': _startHoverMode(_alertDisplayRegistry); break
        case 'banner': _startBannerMode(_alertDisplayRegistry); break
      }
    }
    ...
    function _stopAlertDisplayMode(mode) {
      ...
      switch (mode) {
        ...
        case 'hover': _stopHoverMode(); break
        case 'banner': _stopBannerMode(); break
      }
    }

    function _initAlertDisplayModes(alertRegistry) {
      _alertDisplayRegistry = alertRegistry
      var modes = ['toast', 'dropdown', 'hover', 'banner']
      for (var i = 0; i < modes.length; i++) {
        if (_getAlertDisplayMode(modes[i])) {
          _startAlertDisplayMode(modes[i])
        }
      }
    }
    ...
    return {
      ...
      apply(ctx) {
        try {
          ...
          //    Four independent toggles controlling how alerts are shown
          //    near the ⚡ trigger: Toast, Dropdown, Hover, Banner.
          //    Default: Toast ON, others OFF.
          const _alertDisplayModes = [
            { mode: 'toast', labelKey: 'alertToast', icon: '🔔', order: 58 },
            { mode: 'dropdown', labelKey: 'alertDropdown', icon: '📋', order: 59 },
            { mode: 'hover', labelKey: 'alertHover', icon: '👁', order: 60 },
            { mode: 'banner', labelKey: 'alertBanner', icon: '📢', order: 61 },
          ]
