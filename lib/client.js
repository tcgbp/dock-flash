...
window.__ModuleLoader__.load({
  id: 'dock-flash',
  factory: (require) => {
    console.log('[dock-flash] client v1.0.3')
...
//#region i18n ─────────────────────────────────────────────────────────────────
    ...
    const en = {
      ...
      triggerSidebarFooter: 'Sidebar Footer',
...
//#region PanelComponent ───────────────────────────────────────────────────────
    ...
    function QuickControlPanel(props) {
      ...
      const _closeOnBlurKey = 'dock-flash:close-on-blur'
      const _getCloseOnBlur = () => {
        try {
          const v = localStorage.getItem(_closeOnBlurKey)
          return v === null ? 'off' : v  // default: off
        } catch (_) { return 'off' }
      }
...
//#region StandaloneMode ───────────────────────────────────────────────────────
    ...
    function mountStandaloneSlotTrigger(ctx, registry) {
      ...
      var TRIGGER_POSITIONS = [
        { value: 'sidebar.footer',    slot: 'sidebar.footer.action',                     label: L('triggerSidebarFooter'),      style: 'badge' },
      ...
      function ensurePanelContainer() {
        ...
        //    Reads the same localStorage key the outside-click handler and the
        //    workbench header use, so all three stay in sync.
        var _cobOn = function () {
          try { return localStorage.getItem('dock-flash:close-on-blur') !== 'off' } catch (_) { return false }
        }
        var _cobLabel = function (on) {
          return t('closeOnBlur') + ': ' + t(on ? 'closeOnBlurOn' : 'closeOnBlurOff')
        }
        var _cobPaint = function (on) {
          cobBtn.setAttribute('aria-pressed', String(on))
          cobBtn.setAttribute('aria-label', _cobLabel(on))
          cobBtn.setAttribute('title', _cobLabel(on))
        ...
        cobBtn.addEventListener('mouseleave', function () { _cobPaint(_cobOn()) })
        cobBtn.addEventListener('click', function (e) {
          e.stopPropagation()
          var on = !_cobOn()
          try { localStorage.setItem('dock-flash:close-on-blur', on ? 'floating' : 'off') } catch (_) {}
          _cobPaint(on)
        })
        _cobPaint(_cobOn())
      ...
      function positionPanel() {
        ...
        if (posConfig && posConfig.style === 'badge') {
          // Sidebar footer: panel above the badge, left-aligned with sidebar
          panelContainer.style.bottom = (window.innerHeight - rect.top + 6) + 'px'
          panelContainer.style.top = 'auto'
          panelContainer.style.left = Math.max(8, rect.left) + 'px'
          panelContainer.style.right = 'auto'
        } else if (posConfig && posConfig.style === 'input') {
      ...
      // ── QuickTriggerBadge — React component for sidebar footer slot ──
      //    Matches the CordisPanel badge pattern: icon + label when wide, icon only when rail.
      function QuickTriggerBadge(props) {
        ...
        var btnStyle = {
          ...
          padding: wide ? '4px 10px' : '4px',
          fontSize: wide ? '13px' : '18px',
          lineHeight: 1,
        ...
        var children = [h('span', { style: iconStyle, key: 'icon' }, LightningIcon(wide ? 14 : 18))]
        if (wide) children.push(h('span', { key: 'label' }, t('title')))
      ...
      ctx.inject(['slots'], function (scope) {
        ...
        function injectTrigger(posValue) {
          ...
          var Component = posConfig.style === 'badge' ? QuickTriggerBadge : QuickTriggerIconButton
    ...
    return {
      ...
      apply(ctx) {
        try {
          ...
          // Close-on-blur — NOT a panel switch. It is exposed as a toggle in
          // the panel header, left of the close button, in both modes (see the
          // standalone floating head and the workbench headerComponent below).
          // That is deliberate: with it removed from the registry, no built-in
          // switch carries `group: 'layout'` in workbench mode, so the Layout
          // category disappears there — every layout property it could have
          ...
          const _setCloseOnBlur = (v) => {
            try { localStorage.setItem(_closeOnBlurKey, String(v)) } catch (_) {}
          }
          const _isCloseOnBlurOn = () => _getCloseOnBlur() !== 'off'
          /** Tooltip / aria label for the header toggle, e.g. "Close on Blur: On". */
          const _closeOnBlurLabel = (on) =>
            `${t('closeOnBlur')}: ${t(on ? 'closeOnBlurOn' : 'closeOnBlurOff')}`
          ...
          if (wb) {
              ...
              const dispose = wb.registerPanel({
                ...
                headerComponent: (props) => {
                  ...
                  const cobPaint = (el, on) => {
                    ...
                    el.setAttribute('aria-label', _closeOnBlurLabel(on))
                    el.setAttribute('title', _closeOnBlurLabel(on))
                  }
                  const toggleCob = (e) => {
                    const on = !_isCloseOnBlurOn()
                    _setCloseOnBlur(on ? 'floating' : 'off')
                    cobPaint(e.currentTarget, on)
                  }
                  return h('div', { style: { display: 'flex', alignItems: 'center', gap: '2px' } },
                    h('button', {
                      type: 'button',
                      ref: (el) => cobPaint(el, _isCloseOnBlurOn()),
                      style: { ...S.panelCloseBtn, fontSize: '14px', display: 'inline-flex', alignItems: 'center' },
                      onClick: toggleCob,
                      onMouseEnter: (e) => { e.currentTarget.style.opacity = '1' },
                      onMouseLeave: (e) => cobPaint(e.currentTarget, _isCloseOnBlurOn()),
            ...
            // Inject trigger badge into sidebar footer + floating panel.
