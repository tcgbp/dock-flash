...
window.__ModuleLoader__.load({
  id: 'dock-flash',
  factory: (require) => {
    console.log('[dock-flash] client v1.1.11')
...
//#region i18n ─────────────────────────────────────────────────────────────────
    ...
    const zh = {
      ...
      orderEdit: '排序',
      orderDone: '完成',
      // Names the SCOPE as well as the consequence: the reset is per-tab, so
      // "恢复默认" on its own invited the reading that both tabs would reset.
      orderReset: '恢复本页签的默认顺序（同时清除已保存的排序，包括已禁用插件的记录）',
    ...
    const en = {
      ...
      orderEdit: 'Reorder',
      orderDone: 'Done',
      orderReset: 'Reset this tab\'s order (also forgets saved positions, including those of disabled plugins)',
...
//#region PanelOrder ───────────────────────────────────────────────────────────
    ...
    function clearPanelOrder(scope) {
      ...
      // The localStorage mirror is removed outright only for a full clear; a
      // scoped clear still has a value to keep, so it is written instead.
      savePrefs(_prefCtx, { panelOrder: next }, () => {
        try {
          if (scope === undefined) localStorage.removeItem(PANEL_ORDER_KEY)
          else localStorage.setItem(PANEL_ORDER_KEY, JSON.stringify(next))
        } catch (_) {}
      })
    }
...
//#region PanelComponent ───────────────────────────────────────────────────────
    ...
    function QuickControlPanel(props) {
      ...
      // A momentary receipt for the reset button. Without it the click had NO
      // visible effect at all: the handler only cleared storage and bumped a
      // tick, and a reset usually lands close enough to the previous order that
      // the panel barely moves — so "it worked" and "it did nothing" looked
      // identical. The changelog cannot carry it: that renders in the Changes
      // tab, which does not exist until something has been recorded, so a
      // receipt written there is invisible exactly when it is first needed.
      //
      // The receipt REPLACES THE TAB TITLE for ~1.6s. An earlier version only
      // flashed the button, which said "you clicked" but not what happened —
      // and a reset has a consequence worth naming (it forgets the positions of
      // disabled plugins too). The header is the most prominent line of the tab
      // and already carries text, so the message costs no new chrome. It holds
      // the TAB ID rather than a boolean: only the tab whose button was pressed
      // reports it, and the other keeps its name.
      const [orderResetNotice, setOrderResetNotice] = useState(null)
        ...
        pages.map((page, pi) => {
          ...
          return h('div', { key: page.id, style: isLast ? S.tabPageLast : S.tabPage },
            ...
            h('div', {
              ...
              // While a reset notice is live for THIS tab, the title becomes the
              // message: same slot, same type scale, so nothing reflows beyond the
              // text itself.
              orderResetNotice === page.id
                ? h('span', { style: S.tabPageHeaderNotice }, page.icon, ' ', t('orderResetDone'))
              ...
              h('div', { style: S.tabPageHeaderRight },
                ...
                page.order && orderable
                  ? h('button', {
                      ...
                      onClick: (e) => {
                        e.stopPropagation()
                        // Leaving edit mode also clears the reset receipt: the button is
                        // about to unmount, and a leftover `true` would light it up for no
                        // reason the next time edit mode is entered.
                        setOrderEdit((v) => !v)
                        setOrderResetNotice(null)
                      },
                    }, orderEdit ? '✓' : '⇅')
                  : null,
                page.order && orderable && orderEdit
                  ? h('button', {
                      type: 'button',
                      style: orderResetNotice === page.id ? S.orderIconBtnOn : S.orderIconBtn,
                      // title carries the consequence (it is the only place
                      // that says the reset also forgets disabled plugins);
                      // aria-label stays short because a screen reader reads it
                      // on every focus.
                      title: t('orderReset'),
                      'aria-label': t('orderResetShort'),
                      onClick: (e) => {
                        e.stopPropagation()
                        // Scoped to THIS tab: the Workbench reset must not touch the
                        // Extensions order, and vice versa. `page.id` is the tab id,
                        // which clearPanelOrder maps onto the matching halves of
                        // panelOrder (see its comment).
                        ...
                        setOrderResetNotice(page.id)
                        setTimeout(() => setOrderResetNotice(null), 1600)
                      },
                    }, '↺')
                  : null,
    ...
    return {
      ...
      apply(ctx) {
        try {
          ...
          _prefCtx = ctx
          try { window.__dockFlashPrefs = prefsProbe } catch (_) {}
          // Measuring must be as cheap as looking, or it does not happen.
          try { window.__dockFlashOverflow = overflowProbe } catch (_) {}
