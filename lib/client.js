...
window.__ModuleLoader__.load({
  id: 'dock-flash',
  factory: (require) => {
    console.log('[dock-flash] client v1.0.0')
...
//#region i18n ─────────────────────────────────────────────────────────────────
    ...
    const en = {
      ...
      autoHide: 'Auto Hide',
      autoHideOff: 'Off',
      autoHideEdge: 'Hide on mouse leave',
      on: 'On',
      off: 'Off',
    ...
    return {
      ...
      apply(ctx) {
        try {
          ...
          // Dock position — requires workbench (not available in standalone mode)
          if (wb) {
            ctx.effect(() => {
              const dispose = registry.registerSwitch({
                id: 'dock-flash:dock-position',
                label: L('dockPosition'),
                icon: '📐',
                type: 'buttongroup',
                group: 'layout',
                order: 30,
                options: i18nOptions([
                  ['dockLeft', 'left'], ['dockRight', 'right'],
                  ['dockTop', 'top'],   ['dockBottom', 'bottom'],
                ]),
                getValue: () => {
                  try { return wb.getLayout().dock } catch (_) { return 'right' }
                },
                setValue: (v) => { wb.updateLayout({ dock: v }) },
              })
              return dispose
            }, 'dock-flash: dock-position switch')
          }

          // Auto-hide toggle — requires workbench (not available in standalone mode)
          if (wb) {
            ctx.effect(() => {
              const dispose = registry.registerSwitch({
                id: 'dock-flash:auto-hide',
                label: L('autoHide'),
                icon: '👁️',
                type: 'toggle',
                group: 'layout',
                order: 40,
                getValue: () => {
                  try { return wb.getLayout().autoHide === 'edge' } catch (_) { return false }
                },
                setValue: (v) => { wb.updateLayout({ autoHide: v ? 'edge' : 'off' }) },
              })
              return dispose
            }, 'dock-flash: auto-hide switch')
          }
          ...
          // Close-on-blur toggle — works in both workbench and standalone mode
          const _closeOnBlurKey = 'dock-flash:close-on-blur'
          const _getCloseOnBlur = () => {
            try {
              const v = localStorage.getItem(_closeOnBlurKey)
          ...
          ctx.effect(() => {
            const dispose = registry.registerSwitch({
              ...
              group: 'layout',
              order: 42,
              options: i18nOptions([['closeOnBlurOff', 'off'], ['closeOnBlurFloating', 'floating']]),
              getValue: () => _getCloseOnBlur(),
              setValue: (v) => {
                _setCloseOnBlur(v)
              },
            })
            return dispose
          }, 'dock-flash: close-on-blur switch')
