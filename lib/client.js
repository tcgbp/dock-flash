        ...
        return h('div', { key: 'alert-bar', style: S.alertBar },
          ...panelAlerts.map((alert) =>
            h('div', { key: alert.id, style: { ...S.alertItem, ...severityStyle(alert.severity) } },
              ...
              h('span', { style: S.alertItemText }, alert.title, alert.message ? ': ' + alert.message : ''),
...
//#region StandaloneMode ───────────────────────────────────────────────────────
    ...
    function mountStandaloneSlotTrigger(ctx, registry) {
      ...
      function QuickTriggerIconButton(props) {
        ...
        // Alert badge — rendered conditionally when alerts exist
        var badgeEl = alertCount > 0
          ? h('span', {
              style: S.alertBadge,
      ...
      function mountOverlayTrigger() {
        ...
        // Alert badge — imperative DOM, not React. Shows a red count dot when
        // the alert registry has active alerts. Hidden when count is zero.
        ...
        try {
          ...
          if (_ar) {
            var _paintBadge = function () {
              var count = _ar.alertCount()
              if (count > 0) {
                _overlayBadge.style.display = ''
                _overlayBadge.textContent = count > 99 ? '99+' : String(count)
              } else {
                _overlayBadge.style.display = 'none'
              }
            }
