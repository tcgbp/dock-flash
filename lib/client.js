...
window.__ModuleLoader__.load({
  id: 'dock-flash',
  factory: (require) => {
    console.log('[dock-flash] client v1.0.14')
...
//#region PanelOrder ───────────────────────────────────────────────────────────
    ...
    function readPanelOrder() {
      ...
      try {
        const parsed = JSON.parse(localStorage.getItem(PANEL_ORDER_KEY) || 'null')
    ...
    /** The ONLY writers of the key — set and clear, declared side by side. */
     ...
     *  A unit is bucketed by its head's type — a cluster is never a toggle.
    ...
    function displayUnits(persistedIds, switches, builtIn) {
      ...
      const toggles = ordered.filter((u) => u.type === 'toggle')
      const rest = ordered.filter((u) => u.type !== 'toggle')
      return toggles.concat(rest)
    }
//#endregion ───────────────────────────────────────────────────────────────────
...
//#region PanelComponent ───────────────────────────────────────────────────────
    ...
    function QuickControlPanel(props) {
      ...
      /** One unit outside edit mode: a single switch, or a cluster block whose
       *  members keep their fixed internal order. */
      function renderUnit(unit) {
        if (unit.members.length === 1) return renderSwitch(unit.members[0])
        return h('div', { key: unit.key }, unit.members.map((sw) => renderSwitch(sw)))
      }

      /** One unit inside an editing group. A cluster gets a SINGLE ▲▼ pair beside
       *  the whole block — the unit moves, its members never do. The block's
      ...
      function renderOrderUnit(unit, groupKey, units, index) {
        return h('div', { key: unit.key, style: S.orderRow },
          ...
          h('div', { style: S.orderRowBody }, unit.members.map((sw) => renderSwitch(sw))),
        )
      }
      ...
      function renderBuiltInGroup(groupKey, switches, isFirst) {
        ...
        const toggles = units.filter((u) => u.type === 'toggle')
        const others = units.filter((u) => u.type !== 'toggle')
        ...
        try {
          ...
          ctx.effect(() => {
            const dispose = registry.registerSwitch({
              ...
              label: L('testUrl'),
              // This row is deliberately bare: label + select, nothing else.
              // It used to carry an icon, and the effective URL used to print
              // on its own full-width line (subtitleBlock) directly above the
              // Test Connection button — both removed.
              //
              // Hidden while no proxy variable is configured: with no proxy to
              // route through the probe has nothing to exercise, which is the
              // same condition the System switch's tooltip calls "no effect".
              // Re-evaluated on every render; _fetchProxyStatus notifies
              // 'dock-flash:system-proxy' when the value changes.
              visible: () => !!_httpProxyValue,
              type: 'select',
              group: 'system',
              cluster: 'system-proxy',
              order: 51,
          ...
          ctx.effect(() => {
            ...
            const dispose = registry.registerSwitch({
              ...
              icon: '🌐',
              // Same gate as test-url: with no proxy variable configured there
              // is nothing for this probe to route through.
              visible: () => !!_httpProxyValue,
              type: 'action',
              group: 'system',
              cluster: 'system-proxy',
