...
//#region i18n ─────────────────────────────────────────────────────────────────
    ...
    const zh = {
      ...
      proxyLog: '连接诊断日志',
      proxyLogClear: '清空日志',
      useSystemProxy: '使用 Windows 系统代理',
      useSystemProxyHint: '读取「设置 → 网络和 Internet → 代理」中的手动代理设置，并应用于 DSH 的出站请求',
      winProxySocksOnly: '系统只配置了 SOCKS 代理，无法应用（仅支持 HTTP/HTTPS 代理）',
      winProxyPacOnly: '系统配置了 PAC 脚本，无法应用（代理地址需要执行脚本才能确定）',
      winProxyUnavailable: '无法读取 Windows 系统代理设置',
      logRoute: '链路',
      logProxied: '经代理',
      logDirect: '直连',
    ...
    const en = {
      ...
      useSystemProxy: 'Use Windows System Proxy',
      useSystemProxyHint: 'Read the manual proxy from Settings → Network & Internet → Proxy and apply it to DSH outbound requests',
      winProxySocksOnly: 'Windows configures only a SOCKS proxy, which cannot be applied (HTTP/HTTPS only)',
      winProxyPacOnly: 'Windows configures a PAC script, which cannot be applied (the address needs the script run)',
      winProxyUnavailable: 'Could not read the Windows proxy setting',
      logRoute: 'Route',
      logProxied: 'proxied',
      logDirect: 'direct',
...
//#region SwitchRenderers ──────────────────────────────────────────────────────
    ...
    function renderSwitch(sw) {
      // `visible()` lets a switch opt out of rendering entirely — used by the
      // proxy controls to disappear when no proxy is in play. Guarded here as
      // well as at the group-building site so any other caller is covered.
...
//#region PanelComponent ───────────────────────────────────────────────────────
    ...
    function QuickControlPanel(props) {
      ...
      // Filter before grouping, not while rendering: a group whose every switch
      // is hidden must not render its title above nothing, and
      // `groupOrder.filter((g) => builtInGroups.has(g))` already drops a group
      // nothing was added to.
      const isVisible = (sw) => typeof sw.visible !== 'function' || !!sw.visible()
      const builtIn = allSwitches.filter((sw) => String(sw.id).startsWith('dock-flash:')).filter(isVisible)
      const thirdParty = allSwitches.filter((sw) => !String(sw.id).startsWith('dock-flash:')).filter(isVisible)
    ...
    return {
      ...
      apply(ctx) {
        try {
          ...
          // Environment-derived address ONLY. Kept apart from `_httpProxyValue`
          // because it decides whether the Windows opt-in is offered: the
          // effective value includes the registry as soon as the opt-in is on,
          // so keying off that would hide the toggle the moment it was switched
          // on, leaving no way back.
          var _envProxyValue = null
          // The Windows "Manual proxy setup" descriptor from the host, or null
          // when it has not been fetched yet.
          var _winProxy = null
          var _useSystemProxy = false
          var _proxySource = null
          var _proxyWarning = null
          ...
          /**
           * The Windows opt-in to offer, or null when there is nothing to offer.
           *
           * Only when no proxy variable exists AND Windows actually has one —
           * the whole point is to cover the case where a proxy is configured
           * somewhere Node does not look.
           */
          function _systemProxyOffer() {
            if (_envProxyValue) return null
            if (!_winProxy || _winProxy.platform !== 'win32' || !_winProxy.readable) return null
            var hasServer = !!(_winProxy.httpProxy || _winProxy.httpsProxy || _winProxy.socksProxy)
            // Keep the toggle reachable once the opt-in is on even if the Windows
            // setting disappears underneath it, otherwise it could never be
            // switched back off.
            if (!_useSystemProxy && !(_winProxy.enabled && hasServer)) return null
            return _winProxy
          }

          /** Whether any proxy is in force — env-derived or opted into. */
          function _proxyInEffect() {
            return !!_httpProxyValue
          }
              ...
              fetch('/plugins/dock-flash/proxy-status')
                ...
                .then(function (data) {
                  if (data) {
                    ...
                    if (data.envProxy !== undefined && data.envProxy !== _envProxyValue) {
                      _envProxyValue = data.envProxy
                      changed = true
                    }
                    if (data.systemProxy !== undefined) {
                      // Compare by value: the host re-reads the registry on a TTL,
                      // so a fresh object with identical content must not count as
                      // a change or the panel would repaint on every poll.
                      var winJson = JSON.stringify(data.systemProxy || null)
                      if (winJson !== JSON.stringify(_winProxy)) {
                        _winProxy = data.systemProxy || null
                        changed = true
                      }
                    }
                    if (typeof data.useSystemProxy === 'boolean' && data.useSystemProxy !== _useSystemProxy) {
                      _useSystemProxy = data.useSystemProxy
                      changed = true
                    }
                    if (data.proxySource !== undefined && data.proxySource !== _proxySource) {
                      _proxySource = data.proxySource
                      changed = true
                    }
                    if (data.proxyWarning !== undefined && data.proxyWarning !== _proxyWarning) {
                      _proxyWarning = data.proxyWarning
                      changed = true
                    }
                    ...
                    if (changed) {
                      // Every proxy switch shares this payload, and visibility
                      // depends on it, so all of them are notified.
                      registry.notifyChange('dock-flash:system-proxy')
                      registry.notifyChange('dock-flash:use-system-proxy')
            ...
            if (remote && remote.settings && typeof remote.settings.describe === 'function') {
              remote.settings.describe().then(function (desc) {
                ...
                if (ns && ns.resolved) {
                  ...
                  // The Windows opt-in is host-owned too.
                  if (typeof res.useSystemProxy === 'boolean' && res.useSystemProxy !== _useSystemProxy) {
                    _useSystemProxy = res.useSystemProxy
                    console.log('[dock-flash] use-system-proxy sync: ' + _useSystemProxy)
                  }
                }
              }).catch(function () {})
            }
          ...
          // v1.1.0: Windows "Manual proxy setup" opt-in.
          //
          // Offered only when no proxy variable exists AND the registry has one:
          // that setting is written for WinINET apps, and Node never reads it, so
          // without this the user's proxy is simply invisible here. Off by
          // default because honouring it changes this process's outbound routing
          // to something configured elsewhere.
          ctx.effect(() => {
            const dispose = registry.registerSwitch({
              id: 'dock-flash:use-system-proxy',
              label: L('useSystemProxy'),
              visible: () => !!_systemProxyOffer(),
              icon: '🪟',
              type: 'toggle',
              group: 'system',
              order: 49,
              // The address on its own line: it is the whole point of the switch
              // and is too long for the inline subtitle column.
              subtitle: () => {
                var w = _systemProxyOffer()
                if (!w) return ''
                var addr = w.httpProxy || w.httpsProxy || w.server || ''
                return w.socksProxy ? addr + '   (socks: ' + w.socksProxy + ')' : addr
              },
              subtitleBlock: true,
              tooltip: () => {
                var w = _winProxy
                if (w && !w.httpProxy && !w.httpsProxy && w.socksProxy) return t('winProxySocksOnly')
                if (w && w.pacUrl) return t('winProxyPacOnly')
                if (w && !w.readable) return t('winProxyUnavailable')
                if (_proxyWarning === 'socks-only') return t('winProxySocksOnly')
                if (_proxyWarning === 'pac-only') return t('winProxyPacOnly')
                return t('useSystemProxyHint')
              },
              getValue: () => _useSystemProxy,
              setValue: (v) => {
                _useSystemProxy = !!v
                console.log('[dock-flash] use-system-proxy: ' + _useSystemProxy)
                _suppressHostSync = true
                try {
                  const settings = ctx.get('remote')
                  if (settings && settings.settings) {
                    settings.settings.update('dock-flash', { useSystemProxy: _useSystemProxy })
                      .then(function () {
                        _suppressHostSync = false
                        _fetchProxyStatus()
                      })
                      .catch(function (err) {
                        console.warn('[dock-flash] failed to sync useSystemProxy to host:', err)
                        _suppressHostSync = false
                      })
                  } else {
                    _suppressHostSync = false
            ...
            return dispose
          }, 'dock-flash: use-system-proxy switch')

          ctx.effect(() => {
            const dispose = registry.registerSwitch({
              id: 'dock-flash:system-proxy',
              label: L('systemProxy'),
              // Nothing to configure for a proxy that does not exist — without
              // one, every mode here is inert and the tooltip below even says so.
              visible: _proxyInEffect,
          ...
          ctx.effect(function () {
            var off = ctx.on('settings/updated', function (ns, next) {
              ...
              if (next) {
                ...
                if (typeof next.useSystemProxy === 'boolean' && next.useSystemProxy !== _useSystemProxy) {
                  _useSystemProxy = next.useSystemProxy
                  registry.notifyChange('dock-flash:use-system-proxy')
                  registry.notifyChange('dock-flash:system-proxy')
                  console.log('[dock-flash] use-system-proxy sync from settings/updated: ' + next.useSystemProxy)
                }
              }
          ...
          ctx.effect(() => {
            const dispose = registry.registerSwitch({
              ...
              label: L('testUrl'),
              visible: _proxyInEffect,
              // The effective URL gets its own full-width line below this row —
              // i.e. directly above the Test Connection button. It is a `select`
              // subtitle rather than a tooltip because the URL is the value of
              // this very switch; hiding it behind an ⓘ hover only duplicated it.
          ...
          // multi-line block showing the latest run. Hidden until the first
          // test, so the System group stays compact until there is something to
          // report. The registry changelog cannot do this job: it is
          // single-line and expires after 30s.
          ctx.effect(() => {
            const dispose = registry.registerSwitch({
