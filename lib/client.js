...
window.__ModuleLoader__.load({
  id: 'dock-flash',
  factory: (require) => {
    console.log('[dock-flash] client v1.1.2')
...
//#region Preferences ──────────────────────────────────────────────────────────
    ...
    function loadHostPreferences(ctx) {
      ...
      return settings.describe().then(function (desc) {
        ...
        // The real shape is { writable, hasDocument, namespaces: [...] } — the
        // list lives under `namespaces`, NOT directly on the response. The
        // `ok`/`value` pair this used to read belongs to an older surface and
        // is accepted here too, so a harness on that surface still resolves.
        var list = Array.isArray(desc.namespaces)
          ? desc.namespaces
          : (Array.isArray(desc.value) ? desc.value : null)
        if (!list) {
          return _prefsFail(
                ...
                if (v !== _BH_ID && svc && svc.getTheme().preference === _BH_ID) {
                  ...
                  try {
                    ...
                    if (bhSettings && typeof bhSettings.update === 'function') {
                      bhSettings.update('wxj-theme-black-hole', { theme: 'off' }).catch(() => {})
                    }
                  } catch (_) {}
          ...
          try {
            ...
            if (settingsSvc && typeof settingsSvc.describe === 'function') {
              settingsSvc.describe().then(function (desc) {
                if (!desc) return
                var nsList = Array.isArray(desc.namespaces)
                  ? desc.namespaces
                  : (Array.isArray(desc.value) ? desc.value : [])
