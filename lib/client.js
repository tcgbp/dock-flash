...
window.__ModuleLoader__.load({
  id: 'dock-flash',
  factory: (require) => {
    console.log('[dock-flash] client v1.1.1')
...
//#region Preferences ──────────────────────────────────────────────────────────
    ...
    function loadHostPreferences(ctx) {
      var remote
      try { remote = ctx && ctx.get ? ctx.get('remote') : undefined } catch (e) { remote = undefined }
      if (!remote) {
        return Promise.resolve(_prefsFail(
          'ctx.get("remote") returned nothing',
          'the remote namespace must be declared in inject[] — a plugin that only ctx.get()s it ' +
          'will not have it resolved'))
      }
      if (!remote.settings) {
        return Promise.resolve(_prefsFail(
          'remote.settings is missing',
          'remote resolved, keys=' + Object.keys(remote).join(',') +
          ' — remote.settings must also be declared in inject[] (it is a typert namespace)'))
      }
      var settings = remote.settings
      if (typeof settings.describe !== 'function') {
        return Promise.resolve(_prefsFail(
          'remote.settings.describe is not a function',
      ...
      return settings.describe().then(function (desc) {
        ...
        if (!desc.ok) return _prefsFail('describe() answered ok=false', JSON.stringify(desc.error || desc))
        if (!desc.value) return _prefsFail('describe() carried no value')
