...
window.__ModuleLoader__.load({
  id: 'dock-flash',
  factory: (require) => {
    console.log('[dock-flash] client v1.1.10')
...
//#region i18n ─────────────────────────────────────────────────────────────────
    ...
    const zh = {
      ...
      orderEdit: '排序',
      orderDone: '完成',
      orderReset: '恢复默认顺序（同时清除已保存的排序，包括已禁用插件的记录）',
    ...
    const en = {
      ...
      orderEdit: 'Reorder',
      orderDone: 'Done',
      orderReset: 'Reset order (also forgets saved positions, including those of disabled plugins)',
...
//#region PanelComponent ───────────────────────────────────────────────────────
    ...
    function QuickControlPanel(props) {
      ...
      // identical. The changelog is not an option here: it renders in the
      // Changes tab, which does not even EXIST until something has been
      // recorded, so a receipt written there would be invisible exactly when it
      // is first needed. The receipt is the button's own highlight — no glyph
      // change, because the check mark already means "leave reorder mode" two
      // buttons to the left and a second one would read as the same thing.
      const [orderResetFlash, setOrderResetFlash] = useState(false)
            ...
            h('div', {
              ...
              h('div', { style: S.tabPageHeaderRight },
                ...
                page.order && orderable && orderEdit
                  ? h('button', {
                      type: 'button',
                      style: orderResetFlash ? S.orderIconBtnOn : S.orderIconBtn,
                      ...
                      onClick: (e) => {
                        ...
                        clearPanelOrder()
                        bumpOrder()
                        // Flash even when nothing was stored: the user asked for a reset,
                        // and "there was nothing to reset" is not something they can tell
                        // apart from a dead button.
                        setOrderResetFlash(true)
                        setTimeout(() => setOrderResetFlash(false), 900)
                      },
                    }, '↺')
                  : null,
