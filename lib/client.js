...
window.__ModuleLoader__.load({
  id: 'dock-flash',
  factory: (require) => {
    console.log('[dock-flash] client v1.2.0')
...
//#region PanelComponent ───────────────────────────────────────────────────────
    ...
    function QuickControlPanel(props) {
       ...
       *
       *  A hidden row stays in the list here (that is the entire point — it is
       *  the only place it can be brought back), which is why this mode filters
       *  nothing while every other render surface does. */
      ...
      function renderBuiltInGroup(groupKey, switches, isFirst) {
        ...
        // `allUnits` is the full set in display order; `units` is what this mode
        // draws. Visibility mode draws ALL of them — including the hidden ones,
        // since un-hiding is the only thing that mode is for. Every other surface
        // draws the visible subset, so the normal view and reorder mode agree on
        // what exists and the arrows cannot arrange something the user cannot
        // see. Filtering HERE (not inside displayUnits) keeps the console hook
        // and the persisted-order bookkeeping looking at the full list.
        const allUnits = displayUnits(persistedIds, switches, true)
        const units = visibilityEdit
          ? allUnits
          : allUnits.filter((u) => !isUnitHidden(scoped, u.key))
      ...
      function renderExtGroup(source, switches, gi) {
        ...
        const allUnits = displayUnits(panelOrder.switches[scoped], switches, false)
        const units = visibilityEdit
          ? allUnits
          : allUnits.filter((u) => !isUnitHidden(scoped, u.key))
