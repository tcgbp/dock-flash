...
window.__ModuleLoader__.load({
  id: 'dock-flash',
  factory: (require) => {
    console.log('[dock-flash] client v1.0.8')
...
//#region i18n ─────────────────────────────────────────────────────────────────
    ...
    const zh = {
      ...
      proxyLog: '连接诊断日志',
      proxyLogEmpty: '尚未测试 —— 点击上方「测试连接」开始',
      proxyLogClear: '清空日志',
      logRoute: '链路',
      logProxied: '经代理',
      logDirect: '直连',
    ...
    const en = {
      ...
      proxyLogEmpty: 'No test yet — run Test Connection above',
...
//#region Styles ───────────────────────────────────────────────────────────────
    ...
    const S = {
      ...
      // changelog this does not expire, so diagnostics survive long enough to
...
//#region SwitchRenderers ──────────────────────────────────────────────────────
     ...
     *   emptyText() -> string     (optional) shown while there are no lines
     ...
     *
     * Exists because the changelog is single-line and expires after 30s, which
     * is the wrong shape for diagnostics you want to compare between runs.
     */
    ...
    return {
      ...
      apply(ctx) {
        try {
          ...
          // Capped ring buffer: one test appends several lines, so 200 lines is
          // roughly a dozen runs of history. The registry's changelog cannot
          // serve this purpose — it is single-line and expires after 30s.
          var _proxyLog = []
          var _proxyLogMeta = ''
          var _PROXY_LOG_MAX = 200
          function _logLines(lines) {
            for (var i = 0; i < lines.length; i++) _proxyLog.push(lines[i])
            while (_proxyLog.length > _PROXY_LOG_MAX) _proxyLog.shift()
          }
          function _logStamp() {
            var d = new Date()
          ...
          // v1.0.7: Diagnostics log — a read-only multi-line block keeping the
          // last runs visible. The registry changelog cannot do this job: it is
          // single-line and expires after 30s, which is useless for comparing
          // one run against the next.
          ctx.effect(() => {
            const dispose = registry.registerSwitch({
