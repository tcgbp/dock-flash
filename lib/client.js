...
window.__ModuleLoader__.load({
  id: 'dock-flash',
  factory: (require) => {
    console.log('[dock-flash] client v1.1.12')
...
//#region PanelComponent ───────────────────────────────────────────────────────
    ...
    function QuickControlPanel(props) {
      ...
      // reports it.
      //
      // `kind` distinguishes the two actions that report here: 'reset' (the ↺
      // button — a destructive act with a consequence worth naming) and 'saved'
      // (leaving reorder mode — states a fact the user cannot otherwise confirm).
        ...
        pages.map((page, pi) => {
          ...
          return h('div', { key: page.id, style: isLast ? S.tabPageLast : S.tabPage },
            ...
            h('div', {
              ...
              // same slot, same type scale, so nothing reflows beyond the text itself.
              // Two actions report here, hence the `kind`.
              orderNotice && orderNotice.tab === page.id
                ? h('span', { style: S.tabPageHeaderNotice }, page.icon, ' ',
                    t(orderNotice.kind === 'reset' ? 'orderResetDone' : 'orderSaved'))
              ...
              h('div', { style: S.tabPageHeaderRight },
                ...
                page.order && orderable && orderEdit
                  ? h('button', {
                      ...
                      // The title names the consequence, and the
                      // consequence differs per tab — see the two keys in the
                      // locale table. aria-label stays short because a screen
                      // reader reads it on every focus.
                      title: t(page.id === 'extensions' ? 'orderResetExt' : 'orderReset'),
