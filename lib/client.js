...
//#region Preferences ──────────────────────────────────────────────────────────
    ...
    function _migrateLocalToHost(ctx) {
      ...
      // triggerLayer / overlayOpacity: both have a REAL default that is also a
      // plausible user choice (1150, 0.55), so they follow triggerSize's rule —
      // migrate only when the local value disagrees with the host. Otherwise a
      // user who deliberately picked the default would be migrated over, and a
      // stale browser would overwrite a host value that is already correct.
      var localLayer = _readLocalRaw('dock-flash:trigger-layer')
      if (localLayer && Number(localLayer) !== _hostPrefs.triggerLayer) {
        var parsedLayer = Number(localLayer)
        if (isFinite(parsedLayer) && parsedLayer > 0) patch.triggerLayer = parsedLayer
      }
      var localOpacity = _readLocalRaw('dock-flash:overlay-opacity')
      if (localOpacity && Number(localOpacity) !== _hostPrefs.overlayOpacity) {
        var parsedOpacity = Number(localOpacity)
        if (isFinite(parsedOpacity) && parsedOpacity > 0 && parsedOpacity <= 1) patch.overlayOpacity = parsedOpacity
      }
...
//#region StandaloneMode ───────────────────────────────────────────────────────
    ...
    function mountStandaloneSlotTrigger(ctx, registry) {
      ...
      //
      // The presets are ordered by the MEASURED host UI, read from TWO sources —
      // because reading only one of them is exactly what produced a wrong middle
      // preset the first time:
      //   - DSH's own client bundles top out at **1100** (`dsh-client-ui-chat`,
      //     `dsh-client-ui-model-selection`), with `settings-general`'s overlay
      //     and DSH's menus at 1000.
      //   - **dock-base**, which is NOT under `@deepseek-ai` and was missed on the
      //     first pass, puts its SETTINGS DIALOG at **1100**
      //     (`.dsh-wb-settings-overlay`, deliberately "outside the auto-hide root
      //     so it remains visible and interactive") and its menu at 1000.
      //
      // So 1100 is the ceiling for both the settings dialog and the chat
      // popovers, and a preset meant to sit UNDER them has to be strictly below
      // 1100 — the old 1050 was between the host's two levels, which is why it
      // still covered the settings mask.
      var LAYER_PRESETS = [
        // Under every dialog and menu the host draws (settings 1100, chat 1100,
        // host menus 1000), above ordinary page content. The "stay out of the
        // way" choice; 900 rather than ~1050 because there is no gap to sit in.
        { value: 900, label: '900' },
      ...
      function _loadNumberPref(hostKey, localKey, fallback, validate) {
        ...
        try {
          ...
          if (raw !== null && raw !== '') {
            var n = Number(raw)
            if (isFinite(n) && validate(n)) return n
          }
        } catch (_) {}
        return fallback
      }
      ...
      triggerLayer = _loadNumberPref('triggerLayer', _layerStoreKey, DEFAULT_TRIGGER_LAYER, _layerOk)
      overlayOpacity = _loadNumberPref('overlayOpacity', _opacityStoreKey, DEFAULT_OVERLAY_OPACITY, _opacityOk)
      /** Written on activation change, and reported by the menu. Non-destructive:
       *  a stored value outside the current preset list is kept, so a preset the
       *  client later drops does not silently reset the user's choice. */
