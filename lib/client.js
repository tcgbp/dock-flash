...
//#region StandaloneMode ───────────────────────────────────────────────────────
    ...
    function overlayProbe() {
      ...
      return {
        ...
        offsetSource: (_hostPrefs && _hostPrefs.triggerOverlayOffset) ? 'host' : 'localStorage/default',
        anchorFound: !!vp,
    ...
    function mountStandaloneSlotTrigger(ctx, registry) {
      ...
      function positionPanel() {
        ...
        if (posConfig && posConfig.style === 'input') {
          ...
          // right edge and falling back to ABOVE when there is not room
          // underneath. Without that fallback a button dragged toward the
          // bottom of the conversation would open the panel off-screen.
      ...
      var OVERLAY_SIZE = 24      // matches the input/header trigger button
      var overlayEl = null
      ...
      function positionOverlayTrigger() {
        ...
        var left = vp.contentRight - OVERLAY_SIZE - overlayOffset.dx
        ...
        if (probe.visible && probe.rails && probe.rails.length) {
          ...
          if (railLeft !== Infinity) left = Math.min(left, railLeft - OVERLAY_SIZE - 8)
        }
        ...
        var maxLeft = vp.contentRight - OVERLAY_SIZE
        var minTop = vp.rect.top
        var maxTop = vp.rect.bottom - OVERLAY_SIZE
      ...
      function mountOverlayTrigger() {
        ...
        el.style.cssText = [
          'position:fixed',
          'z-index:99997',                   // just under the panel's 99998
        ...
        el.appendChild(LightningIconNode(16))
        // Faint by default so it does not compete with the conversation, solid
        // on approach — the same reasoning as the panel's own subdued chrome.
        el.addEventListener('mouseenter', function () {
          el.style.opacity = '1'
          el.style.background = 'rgba(127,127,127,0.14)'
        })
        el.addEventListener('mouseleave', function () {
          if (!dragging) el.style.opacity = '0.55'
